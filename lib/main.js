(function() {
  var data, dataDate, dates, actvitySums, pressedButton, chart, deviceSums;
  ...
  Papa.parse("data/sample_data.csv", {
    ...
    header: true,
      ...
      debugger
      _.each(dataDate, function(arr){
        deviceSums.push((_.pluck(arr, 'Activity')).sum());
      });
      totalDeviceUse = deviceSums.sum();
      deviceSums = _.map(deviceSums, function(num){
        return (num / totalDeviceUse * 100);
      });
