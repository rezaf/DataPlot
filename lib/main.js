(function() {
  ...
  var actvitySums, chart, data, dataDate, dataFemale, dataMale, dates,
      desktopSum, deviceSums, mobileSum, pie, pressedButton, tabletSum;
  ...
  function drawCharts(data, pressedButton) {
    ...
    $('#male').html(_(data).where({ Gender: 'male' }).length);
    $('#female').html(_(data).where({ Gender: 'female' }).length);
  ...
  $('.list-group-item').on('click', function() {
    ...
    dataMale = _(data).where({ Gender: 'male' });
    dataFemale = _(data).where({ Gender: 'female' });

    chart.update({
      series: [{ data: activitySums }]
    });
